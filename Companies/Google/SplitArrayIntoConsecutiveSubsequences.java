...
public class SplitArrayIntoConsecutiveSubsequences {
    public boolean isPossible(int[] nums) {
        ...
        for (int i : nums) {
            if (counts.get(i) == 0) {
                continue;
            } else if (tails.getOrDefault(i, 0) > 0) { // appended to a previously constructed consecutive sequence?
                tails.put(i, tails.get(i)-1);
                tails.put(i+1, tails.getOrDefault(i+1, 0)+1);
            } else if (counts.getOrDefault(i+1, 0) > 0 && counts.getOrDefault(i+2, 0) > 0) { // start of a new consecutive sequence?
